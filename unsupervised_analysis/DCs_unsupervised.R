...
DCcells <- getData(gs,"HLADR+LIN-")
trans <- cyto_transformer_extract(gs)


inv <- transformList(names(trans), lapply(trans, `[[`, "inverse"))
DCcells=cytoset_to_flowSet(DCcells)
DCcells<- transform(DCcells, inv)
fs=DCcells
...
panel$marker_class <-ifelse(grepl("(FALSE)",panel$use_channel),"state",ifelse(grepl("(CD45RA)",panel$antigen),"type",ifelse(grepl("(CD45)",panel$antigen),"state",ifelse(grepl("(HLADR)",panel$antigen),"state",ifelse(grepl("(LIN)",panel$antigen),"state","type")))))
...
sce <- cluster(sce, features = "type",
               xdim = xdim, ydim = ydim, maxK = 25, seed = 1234)
...
merging_table1=read.table("merging_DCs.txt",sep="\t",header = T)
...
pdf("DCs_Heatmap.pdf",8,2.2)
  plotExprHeatmap(sce, features = "type", scale="last",col_clust = F,
                  by = "cluster_id", k ="merging1", bin_anno = F,
                  bars = TRUE, perc = TRUE)
  dev.off()
  
  tiff("UMAP_DCs.tiff",width = 8, height = 4,units = 'in',res=200)
  plotDR(sce, "UMAP", color_by ="merging1")
  dev.off()
...
saveRDS(sce, file = "sce_DCs.rds", ascii = FALSE, version = NULL,compress = TRUE, refhook = NULL)
...
write.table(z,paste0("DCs","_unsup_stats.txt"),sep="\t",col.names = NA, quote=F)
