...
panel$marker_class <-ifelse(grepl("(FALSE)",panel$use_channel),"state",ifelse(grepl("(CD45RA)",panel$antigen),"type",
                                                                              ifelse(grepl("(CD45)",panel$antigen),"state",ifelse(grepl("(CD4)",panel$antigen),"type",
                                                                                                                                  ifelse(grepl("(CD3)",panel$antigen),"state",
                                                                                                                                         ifelse(grepl("(CD57)",panel$antigen),"type",
                                                                                                                                                ifelse(grepl("(PD1)",panel$antigen),"type","type")))))))
...
sce <- cluster(sce, features = "type",
               xdim = xdim, ydim = ydim, maxK = 50, seed = 1988)
...
merging_table1=read.table("merging_Tcells.txt",sep="\t",header = T)
...
sce <- mergeClusters(sce, k = clusters, 
 ...
 pdf("Tcells_Heatmap.pdf",12,3.4)
  plotExprHeatmap(sce, features = "type", scale="last",col_clust = F,
                  by = "cluster_id", k ="merging1", bin_anno = F,
                  bars = TRUE, perc = TRUE)
  dev.off()
  
  tiff("UMAP_Tcells.tiff",width = 12, height = 4,units = 'in',res=200)
  plotDR(sce, "UMAP", color_by ="merging1")
  dev.off()
  

library(RColorBrewer)
pdf("ExprHeatmap_merged_last_merging.pdf",15,7)
plotExprHeatmap(sce, features = "type", scale="last",col_clust = F,
...
pdf("UMAP_merged.pdf",20,15)
plotDR(sce, "UMAP", color_by ="merging1")
dev.off()

pdf("merging_strategy.pdf",8,9)
plotExprHeatmap(sce, features = "type", scale="last",col_clust = F,
                by = "cluster_id", k =clusters,m= "merging1", bin_anno = F,
                bars = TRUE, perc = F)
dev.off()

markers <- c("CD45RA","CCR7","PD1","CD57")
pdf("UMAP_markers_merged.pdf",15,10)
plotDR(sce,"UMAP", color_by = markers, ncol = 2,scale = T)
dev.off()
